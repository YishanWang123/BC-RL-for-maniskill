...
class FlowAgent(nn.Module):
    ...
    def forward(self, obs, action):
        ...
        t_b = t[:, :, None]
        u = action - noise
        action_t = t_b * action + (1.0 - t_b) * noise
        v_pred = self.predict_action(obs, self.xt_proj(action_t), t)
    ...
    def get_action(self, obs):
        ...
        B = obs["rgb"].shape[0]
        z = torch.randn(B, self.chunk_size, self.action_dim, device=device)
        ts = torch.linspace(0,1, self.num_steps+1, device=device)
        delta_t = ts[1] - ts[0]
        for i in range(self.num_steps):
            t = torch.full((B, 1), ts[i], device=device)
            v = self.predict_action(obs, self.xt_proj(z), t)
            z = z + v * delta_t
        return z
