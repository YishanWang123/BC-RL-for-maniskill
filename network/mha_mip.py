...
class MIP2StepAgent(nn.Module):
    ...
    def encode_tokens(self, obs):
        ...
        # tokens = torch.cat([img_tok, st_tok, t_tok, x_tok], dim=1)           # (B,3+chunk,d)
        # return self.mem_ln(tokens)
        return img_tok, st_tok
    def bulid_tokens(self, img_tok, st_tok, x_in, t_scalar):
    ...
    def predict_action(self, img_tok, st_tok, x_in, t_scalar):
        """
        x_in: (B,chunk,act)   (I0 or It*)
        t_scalar: (B,1)
        """
        assert x_in.dim() == 3 and x_in.shape[1] == self.chunk_size and x_in.shape[2] == self.action_dim
        assert t_scalar.dim() == 2 and t_scalar.shape[1] == 1

        # tokens = self.encode_tokens(obs, t_scalar, x_in)
        tokens = self.bulid_tokens(img_tok, st_tok, x_in, t_scalar)
        h = self.transformer(tokens)                         # (B,3+chunk,d)
    ...
    def forward(self, obs, action):
        ...
        img_tok, st_tok = self.encode_tokens(obs)

        # step0: I0 = 0, t=0
        I0 = torch.zeros_like(action)
        t0 = torch.zeros((B, 1), device=device)
        a0_pred = self.predict_action(img_tok, st_tok, I0, t0)
        ...
        a_star_pred = self.predict_action(img_tok, st_tok, It, t_star_scalar)
    ...
    def get_action(self, obs):
        ...
        t0 = torch.zeros((B, 1), device=device)
        img_tok, st_tok = self.encode_tokens(obs)
        a0 = self.predict_action(img_tok, st_tok, I0, t0)

        t_star_scalar = torch.full((B, 1), self.t_star, device=device)
        # It = self.t_star * a0                               # 因为 z=0, I0=0 -> It = t* a0
        a = self.predict_action(img_tok, st_tok, a0, t_star_scalar)
        return a